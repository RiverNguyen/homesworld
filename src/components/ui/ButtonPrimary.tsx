...
import { Button } from '@/components/ui/button' // sửa path theo project của bạn
...
const ButtonPrimary: React.FC<SubmitButtonProps> = ({
  ...
  return (
    <Button
      type='submit'
      disabled={isLoading}
      className={` font-halyard-display  flex items-center justify-center gap-2 px-4 py-3 h-10 w-fit rounded-full bg-[#27AAE1] hover:bg-[#42A3CC]  xsm:hover:bg-[#27AAE1] ${className}`}
    >
      {isLoading ? (
        <span>Đang gửi...</span>
      ) : (
