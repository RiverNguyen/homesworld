...
export default function BlogItemCompact({ blog, variant = "compact" }: { blog: PostItem; variant?: Variant }) {
  if (variant === "large") {
    return (
      <article className="w-full h-full group">
        <Link href={`/blog/${blog.slug}`} className="relative flex w-full h-full rounded-[1.125rem] overflow-hidden xsm:rounded-[1rem]">
          <Image
          ...
          <div className="absolute top-4 right-4 flex-center h-7.25 px-3 rounded-[6rem] bg-[#8CC63F] text-white text-[0.875rem] leading-[150%] xsm:h-6.5 xsm:text-[0.75rem]">{blog.taxonomies.category[0].name}</div>
          <div className="absolute left-0 bottom-0 w-full p-4 flex flex-col gap-3">
            <h3 className="text-white text-[1.25rem] font-medium leading-[130%] line-clamp-2 xsm:text-[1.125rem]">{decode(blog.title)}</h3>
            <ICLineDashedWhite className="w-full" />
            <div className="flex-y-center justify-between">
              <span className="text-white/70 text-[1rem] leading-[150%] xsm:text-[0.875rem]">{formatDateDMY(blog.published)}</span>
      ...
      <Link href={`/blog/${blog.slug}`} className="flex-y-center">
        ...
        <div className="flex flex-col gap-3 flex-1 p-4">
          <div className="flex-y-center gap-3">
            <span className="text-[rgba(16,71,95,0.80)] text-[1rem] leading-[150%]">{formatDateDMY(blog.published)}</span>
            <span className="flex-center min-w-22 h-7.25 px-3 rounded-[6rem] bg-[#8CC63F] text-white text-[0.875rem] leading-[150%]">{blog.taxonomies.category[0].name}</span>
          </div>
          <h3 className="text-[#10475F] text-[1.25rem] font-medium leading-[130%] line-clamp-2">{decode(blog.title)}</h3>
        </div>
      </Link>
    </article>
  );
